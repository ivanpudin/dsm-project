...
const Projects = () => {
  ...
  const handleCardClick = async (project: Project) => {
    ...
    try {
      ...
      const [assignedRes, availableRes] = await Promise.all([
        fetch(`/api/project/${project.prid}/employees`, { headers }),
        fetch(`/api/employee/${project.prid}/get-all-filtered`, { headers })
      ])
  ...
  const handleAssign = async (empid: number) => {
    ...
    try {
      const res = await fetch(`/api/employee/${selectedProject.prid}/add`, {
        method: 'POST',
  ...
  const handleRemove = async (empid: number) => {
    if (!selectedProject) return
    try {
      const res = await fetch(`/api/employee/${selectedProject.prid}/remove`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'x-role': getRole() },
        body: JSON.stringify({ empid })
      })
  ...
  const handleDeleteProject = async () => {
    ...
    try {
      const res = await fetch(`/api/project/${selectedProject.prid}/delete`, {
        method: 'DELETE',
        headers: { 'x-role': getRole() }
      })
      const data = await res.json()
