...
RETURNS TRIGGER AS $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM Project WHERE PrID = OLD.PrID) THEN
        RETURN OLD;
    END IF;
...
CREATE OR REPLACE TRIGGER trigger_project_must_have_workers
...
BEGIN
    IF NOT EXISTS (
        ...
        RAISE EXCEPTION 'Employee % must work on at least one project', OLD.EmpID;
    END IF;
    RETURN OLD;
END;
$$ LANGUAGE plpgsql;

CREATE OR REPLACE TRIGGER trigger_employee_must_have_project
AFTER DELETE ON Works
FOR EACH ROW
...
RETURNS TRIGGER AS $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM Employee WHERE EmpID = OLD.EmpID) THEN
        RETURN OLD;
    END IF;
...
CREATE OR REPLACE TRIGGER trigger_employee_must_have_group
...
CREATE OR REPLACE TRIGGER trigger_employee_must_have_role
AFTER DELETE ON Has
FOR EACH ROW
...
CREATE OR REPLACE TRIGGER trg_validate_work_started
